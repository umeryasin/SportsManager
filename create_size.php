...
{
	$color_name=$_POST['size_name'];
	$sql_get_latest_id="SELECT MAX(pro_size_id) AS pro_size_id FROM `product_size`";
	$run_get_latest_id=mysqli_query($ob->connect(),$sql_get_latest_id);
	$row_get_latest_id=mysqli_fetch_assoc($run_get_latest_id);
	$new_id=$row_get_latest_id['pro_size_id']+1;

	$sql_save_color="INSERT INTO `product_size`(`pro_size_id`, `pro_size_name`) VALUES ($new_id,'$color_name')";
